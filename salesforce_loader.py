...
class SalesforceDataLoader:
    ...
    def get_contract_products(self, account_code: Optional[str] = None) -> pd.DataFrame:
        ...
        SELECT
            ...
            Product__r.Family,
            Product__r.STONE_Class__c,
            Product__r.STONE_Color_Type__c,
            Product__r.Long__c,
            Product__r.Width__c,
            Product__r.High__c,
            Product__r.Packing__c,
            ...
            Contract__r.Account__r.Name,
            Contract__r.Account__r.Account_Code__c,
            Contract__r.Account__r.Customer_Class__c,
            CreatedDate
        ...
        if account_code:
            ...
            query += f" AND Contract__r.Account__r.Account_Code__c = '{safe_code}'"
        ...
        for r in records:
            ...
            # Use dimensions from contract product (actual sold dimensions)
            length = r.get("Length__c") or product.get("Long__c") or 0
            width = r.get("Width__c") or product.get("Width__c") or 0
            height = r.get("Height__c") or product.get("High__c") or 0
            
            volume_m3 = (length * width * height) / 1000000
            area_m2 = (length * width) / 10000
            
            # Get price per m3 (already calculated in Salesforce)
            price_m3 = r.get("Price_m3__c") or 0
            segment = r.get("Segment__c") or "Unknown"
            
            data.append({
                "contract_product_id": r.get("Id"),
                "contract_product_name": r.get("Name"),
                "product_id": r.get("Product__c"),
                "product_name": product.get("Name"),
                "product_code": product.get("ProductCode"),
                "family": product.get("Family"),
                "stone_class": product.get("STONE_Class__c"),
                ...
                "discount_pct": r.get("Discount__c"),
                "total_price_usd": r.get("Total_Price_USD__c"),
                "price_m3": price_m3,
                "segment": segment,
                "m2": r.get("m2__c"),
                "m3": r.get("m3__c"),
                "ml": r.get("ml__c"),
                "tons": r.get("Tons__c"),
                "contract_id": r.get("Contract__c"),
                "contract_name": contract.get("Name"),
                "account_id": contract.get("Account__c"),
                "account_name": account.get("Name"),
                "account_code": account.get("Account_Code__c"),
                "customer_class": account.get("Customer_Class__c"),
                "created_date": r.get("CreatedDate")
            })
